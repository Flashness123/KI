...
%diff(F^N,X,F*X^M):-atomic(X), number(N), M is N-1,diff(F,X,DF).
diff(F^N,X,N*DF*F^M) :- diff(F,X,DF),M is N-1. %DF ist die innere ableitung
%(2x+1)^2 = 4x^2+4x+1,df=8x+4
...
simp(A * B, C) :- simp(A, SA), simp(B, SB), rew(SA * SB, C).
%simp(A * B * sin(C), D*sin(C)) :- simp(A, SA), simp(B, SB), rew(SA * SB, C), D is A*B.
simp(A, B) :- rew(A, B).
simp(A, A).
simp(DF,X,DFS) :- 
